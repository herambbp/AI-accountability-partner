...
app.post("/api/cron/notifications", async (req, res) => {
  try {
    ...
    const { data: schedules } = await supabase
      .from("schedules")
      .select("*, user_profiles!inner(expo_push_token)")
      .eq("is_active", true)
      .eq("hour", currentHour)
    ...
    for (const schedule of schedules || []) {
      const token = schedule.user_profiles?.expo_push_token;
      if (token) {
    ...
    if (currentHour >= 9 && currentHour <= 21) {
      ...
      // Get users with push tokens who have been inactive
      const { data: inactiveUsers, error: activityError } = await supabase
        .from("user_activity")
        .select(
          "user_id, last_active_at, last_checkin_sent_at, user_profiles!inner(expo_push_token)"
        )
      ...
      for (const user of inactiveUsers || []) {
        ...
        // Skip if no token or if we already sent a check-in recently
        if (!token) {
